...
import { getBillReminders, addBillReminder, deleteBillReminder, markBillAsPaid } from '../redux/billsSlice';
import { Modal, Button, Form, Input, DatePicker, Select, notification, Calendar, Row, Col, List, Typography, Tag } from 'antd';
...
const BillReminderComponent = () => {
  ...
  const { billReminders, loading, error } = useSelector((state) => state.billReminders);
  ...
  const handleSubmit = (values) => {
    ...
    dispatch(addBillReminder(billData))
      ...
      .catch(() => {
        notification.error({ message: 'Failed to add bill. Please try again.' });
      });
  };
  ...
  const handleDeleteBill = (id) => {
    dispatch(deleteBillReminder(id))
      ...
      .catch(() => {
        notification.error({ message: 'Failed to delete bill. Please try again.' });
      });
  };

  const handleMarkAsPaid = async(billId) => {
    await dispatch(markBillAsPaid(billId))
    dispatch(getBillReminders())
      ...
      .catch((error) => {
        notification.error({ message: `Failed to mark bill as paid: ${error.message}` });
      });
  };
  ...
  return (
    <div className="px-4">
      ...
      <Button type="default" onClick={showCalendarModal} style={{ marginBottom: '20px', marginLeft: '10px' }}>
        View Bill Calendar
      </Button>
      ...
      <List
        ...
        renderItem={(bill) => (
          <List.Item
            actions={[
              <Button type="link" danger onClick={() => handleDeleteBill(bill.id)}>
                Delete
              </Button>,
              !bill.is_paid ? (
          ...
          >
            <List.Item.Meta
              ...
              description={`Due: ${moment(bill.due_date).format('YYYY-MM-DD')} - Status: ${bill.is_paid ? 'Paid' : 'Unpaid'}`}
            />
          </List.Item>
        )}
      />
      ...
      <Modal
        ...
        footer={null}
      >
        <Form layout="vertical" onFinish={handleSubmit} initialValues={billForm}>
          <Form.Item
            label="Bill Name"
            name="bill_name"
          ...
          <Form.Item
            label="Amount"
            name="amount"
            rules={[{ required: true, message: 'Please input the bill amount!' }]}
          >
          ...
          >
            <DatePicker onChange={(date) => setBillForm({ ...billForm, due_date: date })} />
          </Form.Item>

          <Form.Item label="Recurring Interval" name="recurring_interval">
            <Select onChange={(value) => handleSelectChange(value, 'recurring_interval')}>
